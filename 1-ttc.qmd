...
## Question 3 — Prepare the labels to use them wityh `ttc`

``torchTextClassifiers`` provides the ``ValueEncoder`` object to deal with textual labels. You simply need to feed the fitted `LabelEncoder` to the ``ValueEncoder``.

Create a `ValueEncoder` ..... TO BE COMLETED BY MEILAME
...
You are going to use a `WordPieceTokenizer` from the `torchTextClassifiers` package (see the [documentation](https://inseefrlab.github.io/torchTextClassifiers/api/tokenizers.html)). Train a `WordPieceTokenizer` from scratch on the training labels with `vocab_size=5000`
and `output_dim=10`. Use the fitted tokenizer to tokenize one observations (with the `tokenize` method), and inspect the result to understand how the text is split.
...
Training a neural network means finding the weights that minimise a **loss function** — a measure of how wrong the model's predictions are. At each step, the model makes predictions on a **mini-batch** of examples, computes the loss, and updates the weights in the direction that reduces it (via **gradient descent**).
...
Define a `TrainingConfig` and train on a small subsample so it completes quickly. __Use only 1 epoch for fast training__. A model trained on the full dataset for many epochs is loaded from MLflow in the next step. Have a careful look at the hyperparameters. Do you understand what they do? You can see the `TrainingConfig` documentation [here](https://inseefrlab.github.io/torchTextClassifiers/api/configs.html#trainingconfig).
...
<details>
<summary>Clue here</summary>
Create a `TrainingConfig(lr=..., batch_size=..., num_epochs=...)`.
</details>
...
We provide a **pretrained model** trained on the full dataset with many epochs.
Load it as such:
...
Once the model is trained, we can use it to predict NACE codes for new text descriptions.
To understand which tokens drove a prediction, we use **captum's `LayerIntegratedGradients`**,
applied to the model's embedding matrix. This method computes the gradient of the output
with respect to each input token embedding, integrated along a path from a baseline — attributing
a score to each token that reflects its contribution to the predicted class.

::: {.callout-important}
## On the interpretation of integrated gradients
`LayerIntegratedGradients` provides a more faithful attribution than attention weights: it measures
how much each token embedding actually influenced the model's output, not merely which tokens the
model attended to. That said, attributions depend on the choice of baseline and should still be
treated as an exploratory tool rather than a definitive causal explanation.
:::
...
Sample some text from the `X_text` array in a variable called `example_texts`.
Call `ttc.predict(np.array(example_texts), top_k=5, explain_with_captum=True)`. The result is a dict with keys `prediction`, `confidence` and `captum_attributions` among others.
...
Extract `captum_attributions` from the results, then use `map_attributions_to_word()`, `map_attributions_to_char()`, `plot_attributions_at_word()`, and `plot_attributions_at_char()` from `torchTextClassifiers.utilities.plot_explainability`.
...
- **Split** the data into train, validation, and test sets, and encode the string labels to integers with `LabelEncoder`.
